...
export default function Creators() {
  ...
  return <main className="shell">
    <div className="creator-top">
      ...
      <section className="card rising"><div className="section-head"><span className="label">Rising creators</span><button className="view-all" type="button" onClick={()=>setChartExpanded(true)}>View all</button></div>{creators.slice(2).map((x,i)=><div className="mini-row" key={x[0]}><img src={x[1]} alt={`${x[0]} portrait`}/><span className="serif">{x[0]}<small className="meta" style={{display:'block'}}>Director · ▲ {15-i*2} this week</small></span><span className="pink">{58+i*5}</span></div>)}</section>
    </div>
