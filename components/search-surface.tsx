...
export function SearchSurface() {
  ...
  const resultLinks = useMemo(() => clips.slice(0, 5).map((clip) => ({title: clip[0], href: `/clips/${clip[0].toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "")}`})), []);
  ...
  const close = () => { router.push("/"); window.setTimeout(() => document.querySelector<HTMLElement>("[aria-label='Open search']")?.focus(), 50); };
  return <main className="search-surface" role="dialog" aria-modal="true" aria-labelledby="search-title">
    ...
    {tab !== "CREATORS" && <><div className="section-head"><h2 className="serif">◉　Clips</h2><span className="view-all">View all clips　›</span></div><div className="search-results">{clips.slice(0, 5).map((item, index) => <div data-search-highlight={highlight === index ? "true" : "false"} className={highlight === index ? "keyboard-highlight" : ""} key={item[0]}><ClipCard item={item}/></div>)}</div></>}
    {tab !== "CLIPS" && <><div className="section-head section"><h2 className="serif"><Users size={20}/> Creators</h2><span className="view-all">View all creators　›</span></div><div className="search-results">{creators.slice(0, 5).map((item) => <CreatorCard key={item[0]} name={item[0]} image={item[1]} followers={item[2]}/>)}</div></>}
