...
- External re-audit: `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` are missing; Vercel is authenticated but the available projects are `lumina-intuitive-healing` and `wizupxyz`, with no linked HOTRANK project.
- Current HEAD validation: typecheck PASS, build PASS, `git diff --check` PASS, fresh browser protected-action check PASS with zero console errors.
